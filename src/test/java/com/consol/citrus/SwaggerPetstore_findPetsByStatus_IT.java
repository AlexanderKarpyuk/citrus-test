 ...
 * @since 2021-03-19
 */
public class SwaggerPetstore_findPetsByStatus_IT extends TestNGCitrusSupport {
    @CitrusTest
    @Test
    @Parameters("runner")
    public void swaggerPetstore_findPetsByStatus_IT(
            @CitrusResource @Optional TestCaseRunner runner) {
        runner.run(echo("TODO: Code the test SwaggerPetstore_findPetsByStatus_IT"));

        runner.run(http().client("httpClient")
            .send()
            .get(InterceptorHandler.getPath("/v2/pet/findByStatus"))
            .contentType("application/json")
            .queryParam("status", "citrus:randomString(10)")
        );
